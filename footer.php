...
$permission = isset($user->user_info['user_access']) ? @unserialize($user->user_info['user_access']) : array();
...
if(PG_DEBUG && is_object($_benchmark) && $user->user_exists && $user->user_info['user_group'] == 1)
{
	...
	$smarty->assign('debug_cache', $debug_cache);
  	...
  	// Save logging info
  	file_put_contents(PG_ROOT.'/log/user_'.$_benchmark->getUid().'.html', $smarty->fetch('user_debug.tpl'));
}
exit();
?>