...
{if $task=="view"}
	<div class="row">
		<div class="col-xs-12">
			<form class="form-horizontal" name="userForm" method="post" action="{$page}.php">
				<div class="row">
									...
									<div class="widget-main">
										...
										<div class="form-group">
											<div class="col-sm-12">
												...
												<button class="btn btn-sm" onclick="document.getElementById('search').value='';document.getElementById('filter_catid').value=0;document.getElementById('filter_status').value=3;document.getElementById('filter_city_id').value='';document.getElementById('filter_district_id').value='';document.getElementById('limit').value='50';document.userForm.p.value=1;"><i class="icon-undo bigger-110"></i> Làm lại</button>
											</div>
										</div>
									</div>
								</div>
							...
							<div class="widget-body">
								<div class="widget-body-inner" style="display: block;">
									<div class="widget-main">
										<div class="form-group">
											<div class="col-sm-12">
												<select onchange="document.userForm.submit();" size="1" class="form-control" id="filter_group" name="filter_group">
										...
										<div class="form-group">
											<div class="col-sm-12">
												<select onchange="document.userForm.submit();" size="1" class="form-control" id="filter_status" name="filter_status">
						...
						<table id="sample-table-1" class="table table-striped table-bordered table-hover">
							...
							{section name=loops loop=$users}
								<tr class="row{if $smarty.section.loops.index%2==0}0{else}1{/if}">
									...
									<td align="center">
										<input type="checkbox" onclick="isChecked(this.checked);" value="{$users[loops].user_id}" name="cid[]" id="cb{$smarty.section.loops.index}">
									</td>
									<td>{$users[loops].user_name}</td>
									<td><a href="user.php?task=edit&id={$users[loops].user_id}" title="Click để sửa thông tin thành viên này">{$users[loops].user_email}</a></td>
									<td>{$users[loops].user_username}</td>
									<td align="left"">{$users[loops].user_group}</td>
									<td align="left">
										<div class="user_access">
											{foreach from=$users[loops].user_access key=k item=access}
												{if $access}
													<u><b>{$k}</b></u><br>
													{$access}<br>
												{/if}
											{/foreach}
											{if $users[loops].user_user_group != 1}
												<div class="readmore"><a id="userID{$users[loops].user_id}" data-uiid="{$users[loops].user_id}" class="extend" href="#">Chi tiết</a></div>
											{/if}
										</div>
									</td>
									<td align="left">
										{foreach from=$users[loops].user_site key=k item=site}
											{if $k>0}, {/if}<font color="#454545">{$site}</font>
										{/foreach}
									</td>
									<td align="center">{$users[loops].user_lastvisitDate|date_format:"%d/%m/%Y %H:%M:%S"}</td>
									<td align="center">
										{if $users[loops].user_enabled == 1}
...
{elseif $task=="add"}
	<div class="row">
		<div class="col-xs-12">
			...
			<form class="form-horizontal" role="form" action="{$page}.php" method="post" name="userForm" id="userForm" enctype="multipart/form-data">
				<div class="row">
					<div class="tabbable">
						<div class="row">
							<div class="col-xs-5">
										...
										<div class="widget-body-inner">
											<div class="widget-main">
												<div class="form-group">
													...
													<div class="col-sm-9">
														<input type="text" name="user_name" id="user_name" value="" class="col-xs-12 required" maxlength="150" placeholder="Họ và tên" />
													</div>
												</div>
												...
												<div class="form-group">
													...
													<div class="col-sm-9">
														<input type="text" name="user_email" id="user_email" value="" class="col-xs-12 required" maxlength="100" placeholder="Địa chỉ email">
													</div>
												</div>
												<div class="space-4"></div>
												<div class="form-group">
													...
													<div class="col-sm-9">
														<input type="text" name="user_username" id="user_username" value="" class="col-xs-12 required" maxlength="100" placeholder="Tên đăng nhập">
													</div>
												</div>
												<div class="space-4"></div>
												<div class="form-group">
													...
													<div class="col-sm-9">
														<input type="password" name="user_password" id="user_password" value="" class="col-xs-12 required" maxlength="100" placeholder="Mật khẩu">
													</div>
												</div>
												<div class="space-4"></div>
												<div class="form-group">
													...
													<div class="col-sm-9">
														<select onchange="objUser.onchangeGroup()" size="1" class="form-control" id="cbo_group" name="cbo_group">
															<option {if $actived==0}selected="selected"{/if} value="0">- Nhóm thành viên -</option>
															{foreach from=$arrGroup key=gId item=group}
																{if $gId > $userInfo.user_group || $userInfo.user_group==1}
																	<option value="{$gId}"{if $gId==$filter_group} selected{/if}>{$group}</option>
																{/if}
															{/foreach}
														</select>
															...
															{foreach from=$pages key=kp1 item=access}
																<label style="font-size: 13px;" for="{$kp}_{$kp1}">
																	{if $userInfo.user_group>1}
																		{foreach from=$userAccess key=kp2 item=access2}
																			{if $kp==$kp2}
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<form class="form-horizontal" role="form" action="{$page}.php" method="post" name="userForm" id="userForm" enctype="multipart/form-data">
										...
										<div class="widget-body-inner">
											<div class="widget-main">
												<div class="form-group">
													<label class="col-sm-3 control-label no-padding-right" for="form-field-fullname"> Họ và tên </label>
													<div class="col-sm-9">
														<input type="text" name="user_name" id="user_name" value="{$users.user_name}" class="col-xs-12 required" maxlength="150" placeholder="Họ và tên" />
													</div>
												</div>
												<div class="space-4"></div>
												<div class="form-group">
													<label class="col-sm-3 control-label no-padding-right" for="form-field-email"> Địa chỉ Email</label>
													<div class="col-sm-9">
														<input type="text" name="user_email" id="user_email" value="{$users.user_email}" class="col-xs-12 required" maxlength="100" placeholder="Địa chỉ email">
													</div>
												</div>
												<div class="space-4"></div>
												<div class="form-group">
													<label class="col-sm-3 control-label no-padding-right" for="form-field-username"> Tên đăng nhập</label>
													<div class="col-sm-9">
														<input type="text" name="user_username" id="user_username" value="{$users.user_username}" class="col-xs-12 required" {if $user->user_info.user_group  == 1}readonly='true'{/if} maxlength="100" placeholder="Tên đăng nhập">
													</div>
												</div>
												<div class="space-4"></div>
												<div class="form-group">
													...
													<div class="col-sm-9">
														<input type="password" name="user_password" id="user_password" value="" class="col-xs-12 required" maxlength="100" placeholder="Đổi mật khẩu">
													</div>
												</div>
												<div class="space-4"></div>
												<div class="form-group">
													...
													<div class="col-sm-9">
														<select onchange="objUser.onchangeGroup()" size="1" class="form-control" id="cbo_group" name="cbo_group">
															<option {if $users.user_group==0}selected="selected"{/if} value="0">- Nhóm thành viên -</option>
															{foreach from=$arrGroup key=gId item=group}
																{*{if $gId >= $users.user_group || $users.user_group==1}*}
																<option value="{$gId}" {if $gId==$users.user_group} selected="selected"{/if}>{$group}</option>
																{*{/if}*}
															{/foreach}
														</select>
													</div>
												...
												<div class="form-group">
													...
													<div class="col-sm-9">
														<select name="user_enabled" id="user_enabled" class="form-control">
															<option value="0" {if $users.user_enabled == 0}selected='selected'{/if}>Không được phép hoạt động</option>
															<option value="1" {if $users.user_enabled == 1}selected='selected'{/if}>Được phép hoạt động</option>
														</select>
													</div>
												</div>
											</div>
				...
				<input type="hidden" value="{$userId}" name="id" class="user_id" id="user_id">
			</form>
		</div>
	</div>
{/if}
...
function submitform(pressbutton){
	var action = document.userForm.action.value;
	if (pressbutton == 'save') {
		if (action == 'add') {
		...
		if (pressbutton) {
			document.userForm.task.value=pressbutton;
		}
	...
	objUser = {
		onchangeGroup: function() {
			...
			if (group > 1) {
				$("#rSite").css("display", "");
				$("#rAccess").css("display", "");
				objUser.processActionGroupChange(url,group,user_id);
			}else {
		...
		processActionGroupChange: function(url,group_id,user_id) {
